#!/usr/bin/env python3
"""
Fix for MIVAA job progress tracking issues - Version 2
This script fixes the progress callback to properly update main job progress
"""

import re

def fix_progress_callback_v2():
    """Fix the progress callback function to properly update main job progress"""
    ...
    new_callback = '''        def progress_callback(progress_percentage: float, current_step: str, details: dict = None):
            ...
            try:
                if job_id and job_id in active_jobs:
                    ...
                    # Calculate overall job progress (document progress + base progress)
                    # For single document jobs, use the document progress directly
                    base_progress = 5.0  # Initial progress
                    document_progress = progress_percentage * 0.9  # Scale to 90% of total
                    overall_progress = base_progress + document_progress
                    
                    # Update main job progress and step
                    job_info["progress_percentage"] = overall_progress
                    ...
                    logger.info(f"📊 Job {job_id} progress updated: {overall_progress:.1f}% - {current_step}")
                else:
                    logger.warning(f"Job {job_id} not found in active_jobs for progress update")
            except Exception as e:
                logger.warning(f"Failed to update document progress: {e}")'''
    ...
    if old_callback in content:
        ...
        print("✅ Fixed progress_callback function v2")
    else:
...
if __name__ == "__main__":
    if fix_progress_callback_v2():
        print("🎉 Progress tracking fix v2 applied successfully!")
    else:
        print("❌ Failed to apply progress tracking fix v2")
