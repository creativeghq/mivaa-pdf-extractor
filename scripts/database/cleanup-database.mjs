/**
 * Complete Database & Files Cleanup Script
 *
 * This script:
 * 1. Deletes ALL knowledge base data (chunks, embeddings, products, images, etc.)
 ...
 * 4. Deletes temporary files from /tmp directory
 * 5. Verifies cleanup was successful
 * 6. Reports storage and resource usage
 */
...
const BUCKETS_CONFIG = [
  ...
  { name: 'pdf-documents', excludeFolders: ['*'] },  // Preserve ALL
...
async function clearTempFiles() {
  console.log('\n🗑️  Clearing temporary files from /tmp');
  ...
  try {
    ...
    for (const file of files) {
      // Only delete PDF-related temp files
      if (file.startsWith('tmp') || file.includes('pdf') || file.includes('image')) {
        try {
...
async function main() {
  ...
  console.log('\n🗑️  STEP 3: Clear temporary files');
  results.tempFiles = await clearTempFiles();
  ...
  console.log(`✅ Temp files deleted: ${results.tempFiles.deleted}`);
  console.log(`🔒 Storage files preserved: ${totalFilesSkipped}`);
