...
class ClaudePriceSearchService:
    ...
    def _build_prompt(
        ...
        return (
            ...
            f"Use web search to find up to {limit} distinct retailers selling this exact product. "
            "Prioritize, in this order: Google Shopping / Sponsored Product listings, then top organic "
            "retailer results. Avoid price-comparison aggregators (PriceRunner, Idealo, etc.), blog posts, "
            "forums, and marketplaces (Amazon, eBay) unless no other data is available.\n\n"
            f"{local_hint}\n\n"
            "HARD REQUIREMENTS for each result:\n"
            "  1. Retailer's OWN site only (no aggregators).\n"
            "  2. A direct product page URL — NOT a search results page, category page, or homepage.\n"
            "  3. A visible numeric price with a clear currency. Skip 'contact for quote' / 'price on request' results.\n"
            "  4. Prefer the exact size match when dimensions are specified; only substitute sizes if close "
            "     (±10%) and annotate it in the `notes` field.\n\n"
            "After your web searches, call the `submit_price_results` tool EXACTLY ONCE with the final "
            "deduplicated list (same retailer = one entry, lowest price variant). If you cannot find any "
            "qualifying results, call the tool with an empty `retailers` array — do NOT invent data."
        )
