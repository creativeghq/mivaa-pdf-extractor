...
class ClaudePriceSearchService:
    ...
    def _build_prompt(
        ...
        return (
            f"Find published retail prices for {product_spec} in {country}. "
            f"Return up to {limit} results — any retailer with a VISIBLE NUMERIC PRICE on their page, "
            "regardless of current stock status.\n\n"
            "What counts as a valid row:\n"
            "- Price visible on the page: YES (even if marked 'Out of stock' / 'Sold out' / "
            "  'Temporarily unavailable'). Record the availability status accurately but still include "
            "  the row — an out-of-stock posted price is valuable market data.\n"
            "- Price on a promo that's clearly been published (was €X, now €Y): YES. Use the current price.\n"
            "- 'Quote on request' / 'Contact for price' / price hidden behind a login: SKIP.\n"
            "- Retailer page that mentions the product but shows no price anywhere on the page: SKIP.\n\n"
            "Before submitting, for each retailer you're about to SKIP as 'no price': do at least one "
            f"follow-up search `{product_spec} site:<retailer-domain>` to see if the retailer has a "
            "specific product page (not just a category or manufacturer-collection page) — product "
            "pages often show a price when category pages don't. If you find the product page with "
            "a price, include that retailer as a valid row.\n\n"
            f"{country_hint}"
