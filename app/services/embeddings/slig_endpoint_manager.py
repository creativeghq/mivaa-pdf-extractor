...
class SLIGEndpointManager:
    ...
    def force_pause(self) -> bool:
        ...
        """
        logger.info("✅ SLIG job completed - letting HuggingFace auto-scale to zero")
    ...
    def manual_pause(self) -> bool:
        ...
        try:
            endpoint.fetch()
            if endpoint.status == "running":
                logger.warning("⚠️ MANUAL PAUSE: SLIG endpoint (will require explicit resume)")
                endpoint.pause()
                self.pause_count += 1
                ...
                self.warmup_completed = False
                logger.info("✅ SLIG endpoint manually paused")
                return True
            else:
            ...
            logger.error(f"❌ Failed to manual pause SLIG endpoint: {e}")
            return False
