    ...
    def scale_to_zero(self) -> bool:
        ...
        try:
            ...
            if current_status == "paused":
                logger.info("SLIG endpoint already paused — no-op")
                return True

            logger.info(f"📉 Pausing SLIG endpoint NOW (was: {current_status}) — instant $0/h")
            endpoint.pause()
            ...
            self.warmup_completed = False
            logger.info(f"✅ SLIG endpoint paused — billing stopped immediately")
            return True
