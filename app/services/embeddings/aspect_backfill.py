...
async def _fetch_stale_aspect_images(
    ...
    if product_id:
        # `product_id` selector pulls images via image_product_associations.
        # Fetch the linked image ids first, then narrow the document_images
        # query — keeps this query single-table and lets the caller index
        # straightforwardly on document_images.id.
        ...
        ids = [r["image_id"] for r in (assoc.data or []) if r.get("image_id")]
        if not ids:
            return []
