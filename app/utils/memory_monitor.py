...
memory_monitor = MemoryPressureMonitor(
    ...
    def trigger_emergency_cleanup(self) -> bool:
        """
        Trigger emergency memory cleanup when memory pressure is critical.
        
        Actions:
        1. Force garbage collection
        2. Clear Python caches
        3. Log memory stats before/after
        ...
        try:
            ...
            logger.warning(f"🚨 EMERGENCY CLEANUP TRIGGERED - Memory: {stats_before.percent_used:.1f}%")
            ...
            logger.info(f"   Before: {stats_before.percent_used:.1f}% ({stats_before.used_mb:.0f}MB)")
            logger.info(f"   After: {stats_after.percent_used:.1f}% ({stats_after.used_mb:.0f}MB)")
            logger.info(f"   Freed: {freed_mb:.0f}MB ({collected} objects collected)")
    ...
    def send_memory_alert(self, stats: MemoryStats, alert_type: str = "warning"):
        """
        Send memory alert to logging/monitoring systems.
        ...
        try:
            ...
            message = f"Memory {alert_type.upper()}: {stats.percent_used:.1f}% used ({stats.used_mb:.0f}MB / {stats.total_mb:.0f}MB)"
            ...
            logger.warning(f"Failed to send memory alert: {e}")


# Global memory monitor instance
global_memory_monitor = MemoryPressureMonitor(
    high_threshold=80.0,
    critical_threshold=90.0,
    check_interval=5.0,
    enable_auto_cleanup=True
